...
const translations = {
    ...
    uk: {
        ...
        errorRoom: "Помилка: Код невірний."
    },
    en: {
...
function init() {
    ...
    peer.on('error', () => alert(translations[currentLang].errorRoom));
}
...
function joinByCode() {
    const code = document.getElementById('manualId').value.toUpperCase().trim();
    if (!code) return;
...
const handleInput = (e) => {
    ...
    else if (conn && conn.open) { conn.send({ x, y }); }
};
...
function update() {
    ...
    [game.p1, game.p2].forEach(p => {
        ...
        if (Math.sqrt(dx*dx + dy*dy) < 38) {
            ...
            game.ball.vx = Math.cos(angle) * 8; game.ball.vy = Math.sin(angle) * 8;
        }
    });
