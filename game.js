...
function startSystem() {
    ...
    peer.on('error', (err) => {
        ...
        if (gameStarted) overlay.style.display = 'flex';
    });
}
...
function reconnectAttempt() {
    if (isHost) {
        if(peer) peer.destroy();
        peer = new Peer(currentRoomId);
        peer.on('open', () => { /* Ждем подключения клиента */ });
...
function gameLoop() {
    ...
    if (gameStarted && now - lastMsgTime > 3000) {
        ...
        if (timeLeft === 0) timerText.innerText = "Пауза. Нажмите RECONNECT когда вернетесь";
    } else {
...
function update() {
    ...
    if (conn && conn.open) conn.send({ state: game, started: gameStarted, type: 'HEARTBEAT' });
}
